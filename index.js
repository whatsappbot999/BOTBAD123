...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Affis Admin\n' 
            + 'ORG: Pengembang XBot;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=19203255902:+19203255902\n' 
            + 'END:VCARD' 
prefix = '!'
blocked = []          
...
client.on('group-participants-update', async (anu) => {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["19203255902@s.whatsapp.net"] 
			...
			switch(command) {
				...
				case 'info':
					...
					break
				case 'listabloq': 
					teks = 'ta aqui todos os meliantes q/dei bloq :\n'
...
client.updatePresence(from, Presence.composing) 
					if (!isQuotedVideo) return reply('❌ responder v�deo hum ❌')
					reply(mess.wait)
          ...
          case 'logoph':
				...
				case 'hobby':
					hobby = body.slice(1)
					const hob =['cozinhar','fazer poha nenhuma','Nadar','sei la pow','Mídia social','ajudar as pessoas','assistir anime','Irritar as pessoasr','desenhar','Cantar','Dançar','passa o dia todo comendo','passa o dia todo dormindo','Tirar fotos','Jogar video game','Falar sozinho']
					const by = hob[Math.floor(Math.random() * hob.length)]
					client.sendMessage(from, 'HOBBY : *'+hobby+'*\n\nTa aqui seu hobby mn : '+ by, text, { quoted: mek })
					break
case 'bot':
		if (args.length < 1) return reply ('vai se ferrar poha me deixa em paz\n parece que e baiano pow\n ou vai se fuder em maluco vai pra casa do caralho em maluco eu te conheço nessa poha? eu sei quem tu é nessa poha? eu sei onde tu mora nessa poha? poha\n brinks ksksksk paz e amor mn')
                            break
  ...
  client.updatePresence(from, Presence.composing) 
      options = {
          text: `「 *SELF WHATSAPP* 」\n\n_Solicitacao by_ : *@${sender.split("@s.whatsapp.net")[0]}\n\nSeu link WhatsApp : *wa.me/${sender.split("@s.whatsapp.net")[0]}*\n*Ou ( / )*\n*api.whatsapp.com/send?phone=${sender.split("@")[0]}*`,
    ...
    client.sendMessage(from, options, text, { quoted: mek } )
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
				case 'ttp':
					if (args.length < 1) return reply('Cade o texto um?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
                ...
                case 'reportarbug':
                    ...
                    client.sendMessage('19203255902@s.whatsapp.net', options, text, {quoted: mek})
...
case 'play':   
               ...
               if (anu.error) return reply(anu.error)
                ...
                case 'antilinkgroup':
					...
					if (Number(args[0]) === 1) {
						...
						nzwa.sendMessage(from,`Atencao a todos os membros ativos deste grupo anti-link. ee você enviar um link de grupo, voce sera expulso daqui  grupo`, text)
              ...
              break
				...
				case 'criador':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://www.mediafire.com/convkey/e384/pml0ge5w90vq5yizg.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nome: Sad boy\n contato: wa.me/19203255902\n Nome do bot: E-Grilo\n status do bot: On'})
					break
                case 'shorturl':
           ...
           case 'rebaixar':
			...
			case 'remover':
				...
				case 'clonar':
					...
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('prontinho chefe')
